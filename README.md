...
# Выполнено ДЗ №9
 ...
 - [ ] Использованы плейбуки, хендлеры и шаблоны для конфигурации окружения и деплоя тестового приложения. Подход один плейбук, один сценарий (play)
...
## В процессе сделано:
 - Создан простой плейбук для настройки бд, выполнен пробный и боевой прогон, добавлены переменные
 - В простой плейбук добавлен handler, реагирующий на изменение mongo config file
 - В простой плейбук добавлена настройка приложения, добавлены переменные
 - В простой плейбук добавлен шаблон для deploy приложения, выполнен деплой
 - В простом плейбуке один сценарий разделен на несколько для db, app, deploy tasks 
 - Плейбук из предыдущей задачи разделен на несколько плейбуков
 - изменен provision в Packer - заменены bash-скрипты на Ansible-плейбуки

## Как запустить проект:
 - Например, запустить команду ansible-playbook site.yml

## Как проверить работоспособность:
 - Например, зайти на {{:reddit-app.ip}}:9292
...
## В процессе сделано:
 - Создал роли stage и prod через утилиту ansible-galaxy, изучил структуру стандартной роли
 - Использовал роли из предыдущего пункта для настройки app и db
 - Создал 2 окружения: stage и prod, использовал окружение stage (default)
 - Научился создавать и использовать переменные групп хостов, добавил вывод информации об окружении
 - Правильным образом организовал хранение данных в папке ansible
 - На примере роли jdauphant.nginx научился работать с community-ролями
 - Выполнил СЗ для организации проксирования app на 80 порту
 - Научился пользоваться Ansible Vault для безопасной работы с приватными данными
 - **Выполнил настройку TravisCI - добавил packer validate
...
# Выполнено ДЗ №11
 ...
 - [ ] Выполнена локальная разработка при помощи Vagrant, доработка ролей для провижининга в Vagrant
 - [ ] Протестированы роли при помощи Molecule и Testinfra

## В процессе сделано:
 - Установлен и настроен Vagrant
 - Описана локальная инфраструктура и создана при помощи этого описания
 - Настроен провиженинг через Ansible
 - Доработаны роли db и app, добавлена параметризация
 - Установлены Molecule и Testinfra
 - На основе Molecule протестирована роль db

## Как запустить проект:
 - Например, запустить команду molecule verify
